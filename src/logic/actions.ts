...
type ActionsWithContext = Parameters<typeof Rune.initLogic>[0]["actions"]

export const actions: ActionsWithContext = {
  ...
  markStockNotStale: (_, { game, playerId }) => {
    if (game.tableaus[getPlayerIndex(game, playerId)].stockIsStale === false) {
      throw Rune.invalidAction()
    }
